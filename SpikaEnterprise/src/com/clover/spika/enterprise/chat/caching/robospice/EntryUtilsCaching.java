		...
		public GlobalModel loadDataFromNetwork() throws Exception {
			...
			if (activity instanceof BaseActivity) {
				...
				if (type == GlobalModel.Type.CHAT) {
					...
					GroupsDao groupDao = ((BaseActivity) activity).getDaoSession().getGroupsDao();
					com.clover.spika.enterprise.chat.models.greendao.Groups group = groupDao.load((long) id);
					...
					UserDao userDao = ((BaseActivity) activity).getDaoSession().getUserDao();
					com.clover.spika.enterprise.chat.models.greendao.User user = userDao.load((long) id);
		...
		public Chat loadDataFromNetwork() throws Exception {
			...
			if (activity instanceof BaseActivity) {
				...
				if (type == GlobalModel.Type.CHAT) {
					...
					com.clover.spika.enterprise.chat.models.greendao.Chat chat = chatDao.load((long) entry.chat.getId());
					...
					com.clover.spika.enterprise.chat.models.greendao.Groups group = groupDao.load((long) entry.group.getId());
					...
					com.clover.spika.enterprise.chat.models.greendao.User user = userDao.load((long) entry.user.getId());
