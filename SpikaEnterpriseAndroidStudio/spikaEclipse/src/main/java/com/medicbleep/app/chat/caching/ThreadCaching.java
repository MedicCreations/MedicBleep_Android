...
import com.medicbleep.app.chat.ThreadsActivity;
...
import com.medicbleep.app.chat.models.greendao.MessageDao.Properties;
...
public class ThreadCaching {
	...
	private static List<Message> getDBData(Activity activity, long id) {
		...
		if (activity instanceof BaseActivity) {
			...
			List<com.medicbleep.app.chat.models.greendao.Message> listOfDaoMessage =
					messDao.queryBuilder().where(Properties.Root_id.eq(id)).build().list();
			
			com.medicbleep.app.chat.models.greendao.Message rootMess = messDao.queryBuilder().where(Properties.Id.eq(id)).build().unique();
...
//				Log.d("LOG", "SAVING TO DATABASE " + mess.getParentId() + ", root: "+mess.getRootId()+", chatid: "+mess.getChat_id()+", id: "+mess.getId());
				...
				com.medicbleep.app.chat.models.greendao.Message finalMessageModel = new com.medicbleep.app.chat.models.greendao.Message(Long.valueOf(mess.id),
