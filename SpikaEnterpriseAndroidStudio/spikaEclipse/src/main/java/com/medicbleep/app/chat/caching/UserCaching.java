...
import com.medicbleep.app.chat.models.greendao.UserDetailsDao;
...
public class UserCaching {
	...
	private static UserWrapper getDBData(Activity activity, String userId, List<UserDetail> wrapperData) {
		...
		if (activity instanceof BaseActivity) {
			...
			com.medicbleep.app.chat.models.greendao.User daoUser = userDao.queryBuilder()
					.where(com.medicbleep.app.chat.models.greendao.UserDao.Properties.Id.eq(userId)).unique();
			...
			if (daoUser != null) {
				...
				com.medicbleep.app.chat.models.greendao.Organization daoOrganization = organizationDao.queryBuilder()
						.where(com.medicbleep.app.chat.models.greendao.OrganizationDao.Properties.Id.eq(daoUser.getOrganization_id())).unique();
				...
				List<UserDetails> lista = userDetailDao.queryBuilder()
						.where(com.medicbleep.app.chat.models.greendao.UserDetailsDao.Properties.User_id.eq(daoUser.getId())).build().list();
				...
				if (lista != null) {
					...
					for (UserDetails userDetails : lista) {
						...
						com.medicbleep.app.chat.models.UserDetail item = handleOldUserDetailsData(userDetails);
	...
	private static com.medicbleep.app.chat.models.UserDetail handleOldUserDetailsData(UserDetails userDetails) {
	...
	private static void handleNewData(Activity activity, UserWrapper networkData) {
		...
		if (activity instanceof BaseActivity) {
			...
			if (networkData.user.organization != null) {
				...
				if (organizationDao.queryBuilder().where(com.medicbleep.app.chat.models.greendao.OrganizationDao.Properties.Id.eq(Long.valueOf(organization.id))).count() > 0) {

					com.medicbleep.app.chat.models.greendao.Organization finalOrganizationModel = organizationDao.queryBuilder()
							.where(com.medicbleep.app.chat.models.greendao.OrganizationDao.Properties.Id.eq(Long.valueOf(organization.id))).unique();
					...
					com.medicbleep.app.chat.models.greendao.Organization finalOrganizationModel = DaoUtils.convertOrganizationModelToOrganizationDao(null, organization);
			...
			if (networkData.user.getDetails() != null) {
				...
				for (Map<String, String> networkDetail : networkData.user.getDetails()) {
					...
					if (userDetailDao
							.queryBuilder()
							.where(com.medicbleep.app.chat.models.greendao.UserDetailsDao.Properties.User_id.eq(networkData.user.getId()),
									com.medicbleep.app.chat.models.greendao.UserDetailsDao.Properties.Key.eq(currentDetailsValue.getKey())).count() > 0) {

						UserDetails userDetails = userDetailDao
								.queryBuilder()
								.where(com.medicbleep.app.chat.models.greendao.UserDetailsDao.Properties.User_id.eq(networkData.user.getId()),
										com.medicbleep.app.chat.models.greendao.UserDetailsDao.Properties.Key.eq(currentDetailsValue.getKey())).unique();
