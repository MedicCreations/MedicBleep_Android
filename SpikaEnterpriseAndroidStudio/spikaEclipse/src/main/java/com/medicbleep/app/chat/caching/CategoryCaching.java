...
import com.medicbleep.app.chat.api.robospice.CategorySpice;
...
import com.medicbleep.app.chat.models.greendao.CategoryDao.Properties;
...
public class CategoryCaching {
	...
	private static CategoryList getDBData(Activity activity) {
		...
		if (activity instanceof BaseActivity) {
			...
			List<com.medicbleep.app.chat.models.greendao.Category> lista = categoryDao.queryBuilder().orderAsc(Properties.Id).build().list();

			if (lista != null) {

				for (com.medicbleep.app.chat.models.greendao.Category cat : lista) {
	...
	private static Category handleOldData(com.medicbleep.app.chat.models.greendao.Category category) {
	...
	private static void handleNewData(Activity activity, List<Category> networkData) {
		...
		if (activity instanceof BaseActivity) {
			...
			for (Category cat : networkData) {
				...
				com.medicbleep.app.chat.models.greendao.Category finalCatModel = new com.medicbleep.app.chat.models.greendao.Category(
