...
import com.medicbleep.app.chat.models.SeenTimestamps;
...
public class ChatCaching {
	...
	private static Chat getDBData(Activity activity, long id) {
		...
		if (activity instanceof BaseActivity) {
			...
			com.medicbleep.app.chat.models.greendao.Chat chatBase = chatDao.queryBuilder().where(Properties.Id.eq(id)).build().unique();
			...
			long tempCount = ((BaseActivity) activity)
					...
					.getMessageDao()
					.queryBuilder()
					.where(com.medicbleep.app.chat.models.greendao.MessageDao.Properties.Chat_id.eq(id),
							com.medicbleep.app.chat.models.greendao.MessageDao.Properties.Root_id.eq(0)).count();

			chat = handleOldData(chatBase);

			if (tempCount != chatBase.getMessageList().size()) {
				List<com.medicbleep.app.chat.models.greendao.Message> tempMess = ((BaseActivity) activity)
						.getDaoSession()
						.getMessageDao()
						.queryBuilder()
						.where(com.medicbleep.app.chat.models.greendao.MessageDao.Properties.Chat_id.eq(id),
								com.medicbleep.app.chat.models.greendao.MessageDao.Properties.Root_id.eq(0)).build().list();
	...
	public static void updateTimestamps (final Activity activity) {
		if (activity instanceof BaseActivity) {
			...
			List<com.medicbleep.app.chat.models.greendao.Message> messages = ((BaseActivity) activity)
					.getDaoSession()
					.getMessageDao()
					.queryBuilder()
			...
			for (com.medicbleep.app.chat.models.greendao.Message message : messages) {
                    ...
                    if(seenTimestamps != null && seenTimestamps.result != null){
                        for (SeenTimestamps.SeenTimestamp seenTimestamp : seenTimestamps.result) {
                            ...
                            com.medicbleep.app.chat.models.greendao.Message finalMessageModel = messageDao.queryBuilder()
                                    .where(com.medicbleep.app.chat.models.greendao.CategoryDao.Properties.Id.eq(seenTimestamp.message_id))
                                    .unique();
