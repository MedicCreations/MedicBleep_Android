...
abstract class AES256Ciphertext {
	...
	static final int MINIMUM_LENGTH_WITH_PASSWORD = HEADER_SIZE + ENCRYPTION_SALT_LENGTH + HMAC_SALT_LENGTH
	...
	AES256Ciphertext(byte[] data) throws InvalidDataException {
		...
		if (version != getVersionNumber()) {
			throw new InvalidDataException(String.format("Expected version %d but found %d.", getVersionNumber(),
					version));
		}
		...
		if (data.length < minimumLength) {
			throw new InvalidDataException(String.format(
					"Data must be a minimum length of %d bytes, but found %d bytes.", minimumLength, data.length));
		}
	 ...
	 */
	private static void validateLength(byte[] data, String dataName, int expectedLength)
			throws IllegalArgumentException {
		if (data.length != expectedLength) {
			throw new IllegalArgumentException(String.format("Invalid %s length. Expected %d bytes but found %d.",
					dataName, expectedLength, data.length));
		}
	}
	...
	byte[] getRawData() {
		...
		if (isPasswordBased) {
			dataSize = header.length + encryptionSalt.length + hmacSalt.length + iv.length + ciphertext.length
					+ hmac.length;
		} else {
		...
		if (isPasswordBased) {
			...
			System.arraycopy(ciphertext, 0, result,
					header.length + encryptionSalt.length + hmacSalt.length + iv.length, ciphertext.length);
			System.arraycopy(hmac, 0, result, header.length + encryptionSalt.length + hmacSalt.length + iv.length
